...
public class UserService {
    ...
    public AdminUserProfileDTO blockProfile(Long adminId, Long userId) {
        ...
        if (userToBlock.isBlocked()) throw new BlockedProfileBlockingException();
    ...
    public List<InternalUserProfileDTO> getTouristsProfiles(Long id) {
        userRepository.findByUserId(id).orElseThrow(() -> new UserNotFoundException(id));
        return userRepository.findByRole(Role.TOURIST).stream()
                .map(InternalUserProfileDTO::fromUser)
                .collect(Collectors.toList());
    }

    public InternalUserProfileDTO getTouristsProfileInternal(Long touristId) {
        User user = userRepository.findByUserId(touristId).orElseThrow(() -> new UserNotFoundException(touristId));
        return InternalUserProfileDTO.fromUser(user);
    }

    public String getUsernameById(Long userId) {
        return userRepository.findByUserId(userId).orElseThrow(() -> new UserNotFoundException(userId)).getUsername();
    }
}
